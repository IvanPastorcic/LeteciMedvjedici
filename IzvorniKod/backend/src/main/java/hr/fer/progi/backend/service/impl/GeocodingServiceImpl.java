...
public class GeocodingServiceImpl implements GeocodingService {
	...
	public SettlementDTO reverseGeocode(float latitude, float longitude) {
		...
		String url = String.format(Locale.US, "https://api.opencagedata.com/geocode/v1/json?q=%f+%f&key=%s", latitude,
				longitude, openCageApiKey);
		...
		try {
			...
			if(!components.path("country").asText().isEmpty() && !components.path("country").asText().equals("Croatia")) {
				throw new RuntimeException("User is not in Croatia");
			}

			String settlement = components.path("_normalized_city").asText();
			if (settlement.isEmpty()) {
				settlement = components.path("city").asText();
			}
			if (settlement.isEmpty()) {
